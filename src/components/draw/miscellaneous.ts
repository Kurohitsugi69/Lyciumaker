...
import { DualFrame } from '../maker/card'
...
export class Miscellaneous {
    ...
    // Lấy thế lực hiệu quả (dùng primaryPower của khung đôi nếu đang dùng, ngược lại dùng power gốc)
    getEffectivePower(card: Card) {
        if (card.dualFrame && card.dualFrame !== '' && DualFrame[card.dualFrame]) {
            return DualFrame[card.dualFrame].primaryPower
        }
        return card.power
    }
}

...
export class AnchorBox {
    ...
    constructor(cvt: CanvasTool, mouse: Mouse, offset = 0, anchorWidth = 15) {
        ...
        this.startSize = new Rect(0, 0, 0, 0)
        this.startMask = new Rect(0, 0, 0, 0)
        this.startC = new Vector(0, 0)
    }

    // Thay đổi vị trí khung neo
    ...
    move(select: 'mask' | 'size') {
        ...
        if (this.moving != 'none') {
            ...
            if (select == 'size') {
                ...
                // Khi ký tự di chuyển, mặt nạ cũng di chuyển theo
                if (this.moving == 'all') {
    ...
    // Lấy các hình chữ nhật tâm tại tám điểm neo
    private getAnchorPoints(rect: Rect): { [key: string]: Rect } {
    ...
    // Kiểm tra chuột có nằm trong hình chữ nhật không
